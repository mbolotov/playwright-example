class IntellijReporter {
  ...
  finishingQueue = new SingleElementQueue(function (testNode) {
    testNode.finish(false);
  });
  curId = undefined;

  onBegin(config, suite) {
    if (this.tree && this.tree.nextId > 0) {
      this.curId = this.tree.nextId
    }
    this.tree = new Tree(function (str) {
      writeToStdout(str);
    });
    if (this.curId) {
      this.tree.nextId = this.curId
    }

    this.tree.writeln('##teamcity[enteredTheMatrix]');
    this.tree.testingStarted();


    var tests = suite.allTests().filter(t => t.title.trim() !== '')

    this.tree.writeln('##teamcity[testCount count=\'' + tests.length + '\']');

    tests.forEach((test) => {
      registerTestNode(this.tree, test);
    });
  ...
  onTestBegin(test) {
    this.finishingQueue.processAll();
    startTest(this.tree, test);
  }
  ...
  onTestEnd(test, result) {
    ...
    if (status === 'timedout' || status === 'interrupted') {
      this.finishingQueue.processAll();
      finishTestNode(this.tree, test, null, this.finishingQueue);
      ...
      //   this.finishingQueue.processAll();
      //   handleBeforeEachHookFailure(this.tree, test, err);
      // }
      // else if (isBeforeAllHook(test)) {
      //   this.finishingQueue.processAll();
      //   finishTestNode(this.tree, test, err);
      //   markChildrenFailed(this.tree, test.parent, test.title + " failed");
      // }
      // else {
        finishTestNode(this.tree, test, result.error, this.finishingQueue);
      // }
    } else {
  ...
  onEnd(result) {
    ...
    // this.finishingQueue.processAll();
      this.tree.root.finishIfStarted()
...
TestNode.prototype.getFinishCommandName = function () {
  switch (this.outcome) {
    ...
    case TestOutcome.ERROR:
      return 'testFailed';
    default:
      throw Error('Unexpected outcome: ' + this.outcome + ":" + this.name);
  }
};
...
function findOrCreateAndRegisterSuiteNode(tree, test) {
  ...
  for (suiteId = 0; suiteId < suites.length; suiteId++) {
    ...
    if (!childNode) {
      ...
      childNode = parentNode.addTestSuiteChild(suiteName, 'suite', locationPath, test.location.file);
      childNode.register();
...
function registerTestNode(tree, test) {
  ...
  testNode = suiteNode.addTestChild(test.title, 'test', locationPath, test.location.file);
  testNode.register();
